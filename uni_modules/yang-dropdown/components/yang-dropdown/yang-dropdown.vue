<template>
	<view class="yang-dropdown" :class="{'isSticky':isSticky, 'isFixed': isFixed, 'overHidden': !show}" :style="{top: isSticky || isFixed ? stickyTop : 0}">
		<view class="dropdow">
			<view v-if="maskVisibility">
				<view class="mock" :class="{'show':show,'hide':maskVisibility!=true}" @click="handClose"></view>
			</view>
			<view class="content" @click="handClose" :style="{height: maskVisibility? maxHei : '78rpx' }">
				<view class="tabs flex align-center">
					<view class="tab flex align-center justify-center" v-for="(item, index) in tableData" :key="index"
						@click.stop="handEnum(index)">
						<text :class="{'active': show && curren === index}">{{ getTitleName(item) }}</text>
						<text :class="{'active-icon': show && curren === index}" class="cuIcon-usefullfill"></text>
					</view>
				...
				<view class="mains" :class="{'open': show}" :style="{ height: mainsHeight }">
					<scroll-view :scroll-y="true" style="height: 100%;">
						<view class="radios">
							<view class="yradio" v-for="(item,index) in child.submenu" :key="index"
								@click="handSelect(item, index)">
								<text :class="{'yradio-active': child.select === index}">{{ item.label }}</text>
								<text v-if="child.select === index" class="cuIcon-check"></text>
							</view>
						</view>
					</scroll-view>
				</view>
